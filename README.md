...
- **Home and profile:** the Earth is NASA's Blue Marble imagery (public domain) wrapped on a sphere by an AGSL shader on Android 13+, tilted, lit from the side and ringed by a blue atmosphere, turning once a minute; older phones and the Moon and Sun themes use a Compose `Canvas` drawing (`ui/Planet.kt`). The profile screen, opened from the top-right icon, holds a sign-in placeholder, the planet choice and the language. Both choices are stored on the device (`AppSettings.kt`).
