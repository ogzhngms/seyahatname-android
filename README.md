...
- **Home and profile:** on Android 13+ the planets are AGSL shaders (`ui/Planet.kt`). The Earth is NASA's Blue Marble imagery and the Moon NASA's LRO colour map (both public domain), wrapped on a tilted, side-lit sphere; the Earth has a blue atmosphere. The Sun is drawn without an image: noise-based granulation, limb darkening and a breathing corona. Older phones get a Compose `Canvas` drawing. The profile screen, opened from the top-right icon, holds a sign-in placeholder, the planet choice and the language. Both choices are stored on the device (`AppSettings.kt`).
